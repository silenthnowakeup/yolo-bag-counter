...
class ObjectCounter:
    ...
    def extract_and_process_tracks(self, tracks):
        ...
        if tracks[0].boxes.id is not None:
            ...
            for box, track_id, cls in zip(boxes, track_ids, clss):
                # Draw bounding box
                self.annotator.box_label(box, label=f"{self.names[cls]}", color=colors(int(track_id), True))  # Removed ID from label
    ...
    def get_in_count(self):
        """Returns the count of objects that have moved in."""
        return self.in_counts

    def get_out_count(self):
        """Returns the count of objects that have moved out."""
        return self.out_counts


if __name__ == "__main__":
    classes_names = {0: "person", 1: "car"}  # example class names
    counter = ObjectCounter(classes_names)
    print(f"In count: {counter.get_in_count()}, Out count: {counter.get_out_count()}")
